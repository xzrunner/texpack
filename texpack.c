...
static inline void
_init_root(struct texpack* tp, int width, int height) {
...
texpack_create(int width, int height, int size) {
	...
	struct texpack* tp = (struct texpack*)malloc(sz);
	memset(tp, 0, sz);
...
static inline struct tp_node*
_insert(struct texpack* tp, struct tp_node* dst, int w, int h, bool can_rotate) {
	assert(dst != NULL);
